...
class Game:
  ...
  def __init__(self):
    ...
    self.win.tracer(0)  #turns off automatic screen update... to manually refresh the screen with update()

    self.shapes = []
    self.score = 0
    self.duration = 60  # seconds game will run
    self.start_time = time.time()

    self.score_text = Text(-200, 250, f"Score: {self.score}")
    self.timer_text = Text(200, 250, f"Time Left: {self.duration}")
  ...
  def move_shapes(self):
    for shape in self.shapes:
      ...
      if shape.shape.ycor(
      ) < -250:  # shape obj has a shape attribute that contains the turtle object
  ...
  def update_timer(self):
    currTime = time.time()
    timePassed = int(currTime - self.start_time)

    timeLeft = self.duration - timePassed
    #timeLeft = max(timeLeft)

    self.timer_text.write_text(f"Time Left: {timeLeft}")
